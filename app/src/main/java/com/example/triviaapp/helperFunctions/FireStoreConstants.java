package com.example.triviaapp.helperFunctions;

public final class FireStoreConstants {

    public static final String ID_FIELD_FIELD = "id";


    //Stats
    public static final String MAIN_STATS_COLLECTION = "top_scores";
    public static final String STATS_SCORE_FIELD = "score";
    public static final String STATS_TIME_SCORE_FIELD = "time_score";
    public static final String STATS_SUBJECT_FIELD = "subject";
    public static final String STATS_TOTAL_GAMES_PLAYED_FIELD = "total_games_played";
    public static final String STATS_TOTAL_CORRECT_ANSWERS_FIELD = "total_correct_answers";
    public static final String STATS_DISPLAY_NAME_FIELD = "display_name";

}
